fun main() {
    ...
    when {
        cardType == "VK Pay" &&  (alreadyPayAmmount + payAmmount) > vkMonthLimit || cardType == "VK Pay" && payAmmount>vkOnesLimit -> println("Превышены лимиты")
        cardType != "VK Pay" && (alreadyPayAmmount + payAmmount) > otherMonthLimit || cardType != "VK Pay" && payAmmount> otherOnesLimit -> println("Превышены лимиты")
...
fun commisionAmount(cardType: String = "VK Pay", alreadyPayAmount: Int = 0 , payAmount : Int) : Double{
    ...
    return when {
        cardType in arrayOf("Mastercard","Maestro") && (alreadyPayAmount + payAmount) > masterMaestroLimit -> if (alreadyPayAmount > masterMaestroLimit) payAmount * masterMaestroComision + masterMaestroAdditionalComision else (alreadyPayAmount + payAmount - masterMaestroLimit)* masterMaestroComision + masterMaestroAdditionalComision
